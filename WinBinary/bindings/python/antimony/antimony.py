...
__antLib.setRemoveFunctionDefinitions.restype = None
__antLib.setRemoveFunctionDefinitions.argtypes = (c_bool, )

def setRemoveFunctionDefinitions(removeFunctionDefinitions):
   """
   Sets whether to remove function definitions on import from SBML.
   By default, is set to 'true'.
   """
   __antLib.setRemoveFunctionDefinitions(removeFunctionDefinitions)

__antLib.getVersionStr.restype = c_char_p
__antLib.getVersionStr.argtypes = ()

def getVersionStr():
   """
   Get the version of Antimony
   """
   ret = __antLib.getVersionStr()
   freeAll()
   if ret==None:
      return ret
...
__antLib.getNumUserFunctions.restype = c_ulong
__antLib.getNumUserFunctions.argtypes = ()

def getNumUserFunctions():
   """
   Returns the number of user-defined functions ('function f(...) ... end' blocks) in the current active set of modules.  Unlike most other queries in this API, this is not scoped to a single module, since Antimony function definitions are not scoped to any one module.
   """
   return __antLib.getNumUserFunctions()

__antLib.getNthUserFunctionName.restype = c_char_p
__antLib.getNthUserFunctionName.argtypes = (c_ulong, )

def getNthUserFunctionName(n):
   """
   Returns the name of the Nth user-defined function.  If no such function exists, NULL is returned and an error is set.
   """
   ret = __antLib.getNthUserFunctionName(n)
   freeAll()
   if ret==None:
      return ret
   return ret.decode('utf-8')
...
__antLib.getNumUserFunctionArguments.restype = c_ulong
__antLib.getNumUserFunctionArguments.argtypes = (c_ulong, )

def getNumUserFunctionArguments(n):
   """
   Returns the number of arguments of the Nth user-defined function.  If no such function exists, 0 is returned and an error is set.
   """
   return __antLib.getNumUserFunctionArguments(n)

__antLib.getNthUserFunctionArguments.restype = POINTER(c_char_p)
__antLib.getNthUserFunctionArguments.argtypes = (c_ulong, )

def getNthUserFunctionArguments(n):
   """
   Returns the names of the arguments of the Nth user-defined function, in the order in which they appear in the function's declaration.  If no such function exists, NULL is returned and an error is set.
   """
   return None
__antLib.getNthUserFunctionBody.restype = c_char_p
__antLib.getNthUserFunctionBody.argtypes = (c_ulong, )

def getNthUserFunctionBody(n):
   """
   Returns the body of the Nth user-defined function, as a formula making use of the function's arguments.  If no such function exists, NULL is returned and an error is set.
   """
   ret = __antLib.getNthUserFunctionBody(n)
   freeAll()
   if ret==None:
      return ret
   return ret.decode('utf-8')
...
def getSymbolDisplayNamesOfType(moduleName, rtype):
   ...
   NOTE:  Two return types repurpose this field for something other than a display name: for allConstraints, this returns the constraint's message text, if any; for the chemical-formula entries in allSpeciesFbcInfo, this returns the chemical formula itself (getSymbolEquationsOfType returns an empty string for those entries--see below).
   """
   if type(moduleName) == str:
      moduleName = moduleName.encode('utf-8')
   ret = []
...
def getSymbolEquationsOfType(moduleName, rtype):
   ...
   - Modules:                 Nothing
   - Constraints:             The right-hand side of the constraint only, as a value or formula.  The comparison operator (>, <, ==, etc.) and left-hand side are not included and cannot currently be retrieved through this API.
   - Unit definitions:        Nothing.  A unit's composition (its kind, exponent, multiplier, and scale) is not retrievable through this API; convert to SBML and use libSBML if you need it.
   - Gene products:           The associated species, if any.
   - Gene product associations: The AND/OR gene product association, as a formula string.  There is no structured (tree) accessor for this; you must parse the string yourself if you need the boolean structure.
   - Species charge (part of allSpeciesFbcInfo):          The charge value.
   - Species chemical formula (part of allSpeciesFbcInfo): Nothing--see getSymbolDisplayNamesOfType instead.
...
def getSymbolAssignmentRulesOfType(moduleName, rtype):
   ...
   @warning Because reactions and genes are included, querying with rtype=allSymbols will return every reaction's kinetic law in this array, not just symbols that actually have assignment rules.  If you want to know whether a model uses assignment rules, query the six symbol classes that can actually carry one (species, formulas, and compartments, each in their const and var forms) instead of allSymbols.
   """
   if type(moduleName) == str:
      moduleName = moduleName.encode('utf-8')
   ret = []
...
__antLib.getSymbolSubstanceOnly.restype = c_bool
__antLib.getSymbolSubstanceOnly.argtypes = (c_char_p, c_char_p, )

def getSymbolSubstanceOnly(moduleName, symbolName):
   """
   Returns 'true' if the given symbol was declared 'substanceOnly' (i.e. its value is defined in amounts rather than concentrations), 'false' otherwise, including if no such symbol or module is found (in which case an error is also set).
   """
   if type(moduleName) == str:
      moduleName = moduleName.encode('utf-8')
   if type(symbolName) == str:
      symbolName = symbolName.encode('utf-8')
   return __antLib.getSymbolSubstanceOnly(moduleName, symbolName)

__antLib.getSymbolHasValue.restype = c_bool
__antLib.getSymbolHasValue.argtypes = (c_char_p, c_char_p, )

def getSymbolHasValue(moduleName, symbolName):
   """
   Returns 'false' if the given symbol was declared with no value at all (as can happen with an SBML parameter with no 'value' attribute, or a compartment with no 'size'), and 'true' otherwise, including for symbols whose value is zero.  This lets you distinguish "no value was given" from "the value is zero" when a symbol's equation (as returned by the functions above) is empty.  Also returns 'false' if no such symbol or module is found, in which case an error is also set.
   """
   if type(moduleName) == str:
      moduleName = moduleName.encode('utf-8')
   if type(symbolName) == str:
      symbolName = symbolName.encode('utf-8')
   return __antLib.getSymbolHasValue(moduleName, symbolName)
...
__antLib.getReactionNames.restype = POINTER(c_char_p)
__antLib.getReactionNames.argtypes = (c_char_p, )

def getReactionNames(moduleName = None):
   """
   Returns the names of the reactions (including genes) in the module.  Is the same as 'getSymbolNamesOfType(moduleName, allReactions)', but is provided for convenience.
   """
   if moduleName==None:
      moduleName = getMainModuleName()
   if type(moduleName) == str:
      moduleName = moduleName.encode('utf-8')
   return None
__antLib.getNthReactionName.restype = c_char_p
__antLib.getNthReactionName.argtypes = (c_char_p, c_ulong, )

def getNthReactionName(moduleName, n):
   """
   Returns the name of the Nth reaction (including genes) in the module.  Is the same as 'getNthSymbolNameOfType(moduleName, allReactions, n)', but is provided for convenience.  If no such reaction exists, NULL is returned and an error is set.
   """
   if type(moduleName) == str:
      moduleName = moduleName.encode('utf-8')
   ret = __antLib.getNthReactionName(moduleName, n)
   freeAll()
   if ret==None:
      return ret
   return ret.decode('utf-8')
