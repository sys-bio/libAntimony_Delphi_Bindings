...
#define LIBANTIMONY_VERSION_STRING "v3.2.0"
#endif
...
/**
 * Returns the stoichiometry for the Mth reactant of the Nth reaction in the module, as it was written, as a string.  If the stoichiometry was written as a plain number, that number is returned as text.  If it was written as a symbol (as in 'reaction1: S1 + n S2 => S3; n = 2*p1;'), the symbol's name is returned, and its value can then be looked up like any other symbol's.  This is provided because getNthReactionMthReactantStoichiometries returns 'NaN' in the latter case, with no way to recover what was actually written.  If no such reactant or reaction exists, an error is set and NULL is returned.
 */
...
/**
 * Returns the stoichiometry for the Mth product of the Nth reaction in the module, as it was written, as a string.  See getNthReactionMthReactantStoichiometryString for details.  If no such product or reaction exists, an error is set and NULL is returned.
 */
